...
class OpenAIClient:
    ...
    def __init__(self, api_key: str, model: str = "gpt-4o-mini"):
        ...
        self.model = model
        self.max_retries = 2  # 减少重试次数：2次（共3次尝试：1次原始 + 2次重试）
    ...
    def generate_json(
        ...
        for attempt in range(self.max_retries):
            try:
                # 只对支持 JSON mode 的模型启用
                use_json_mode = (
                    self.model.startswith("gpt-4") or 
                    self.model.startswith("gpt-3.5-turbo-1106") or
                    self.model == "gpt-3.5-turbo"
                )
                
                logger.info(f"OpenAI API call attempt {attempt + 1}/{self.max_retries} (model: {self.model}, json_mode: {use_json_mode})")
